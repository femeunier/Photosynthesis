...
And now the package should be available for loading:
```{r, eval=TRUE}
...
0. First explore the package functions. To do so, you can either call a function without the parentheses (for example, `Photosynthesis::Photosynthesis`) or write the function in a script and left-click + ctrl on the function name. The core is the `canopy_fluxes` function that essentially takes care of all the steps described above. In particular, it calls the `Photosynthesis` function that solves the system of equations for each layer of vegetation. In addition `canopy_fluxes` aggregates the results for the entire canopy. Also pay attention to the `FvCB.model` function that should be similar to the one you wrote yourself during the last practical.
...
# For Step 5, you need to modify the canopy_fluxes function and explore the new outputs here.
```

Follow the workflow and see if you understand the steps explored and the different call to the functions. Draw (manually) schematic worflow
