...
During this second practical, we will further develop our leaf model to account for stomatal regulation and the vertical intragration of processes along the canopy. The Farquhar, von Caemmerer and Berry model (FvCB) model developped during Practical A is now coupled to a stomatal conductance model (the Ball-Berry model) and the light extinction model from Monsi and Saeki (1953) following an exponential Beer-Lambert law (Slide 15 of Lecture 3). 
...
In practical A, we re-coded the FvCB for C3 photosynthesis. For reminder, this model computed the net assimilation rate ($A_n$) from the internal concentration in CO<sub>2</sub> ($c_i$) and atmospheric (e.g. incoming radiation) and leaf variables (e.g. leaf temperature) and parameters ($V_{cmax}$):
...
To do so, we implemented a second set of equations to reproduce the attenuation of the light into the canopy. The coefficient of light extinction into the canopy follows a Beer-Lambert law as described in Monsi and Saeki (1953). This representation is still commonly used in big-leaf models since it is efficient in term of computing ressources, and sufficient for several research questions which do not require an explicit representation of the canopy (refers to slide 15 and 16 of Lecture 3):
$$
I^{\downarrow}=I^{\downarrow}_{sky}e^{-K_bL}
$$
with $I^{\downarrow}$ the incident radiation reaching the LAI layer $L$ which depends on the direct beam extinction coefficient $K_b$. $I^{\downarrow}_{sky}$ represents the incoming solar radiation.

This model simulates light transmission into an homogeneous medium without scattering. Practically, the canopy, represented by its leaf area index (LAI) is decomposed into several layers along the vertical profile. This decomposition accounts for the distribution of leaves. For each layer we compute the extinction coefficient of light. This coefficient will then be applied to the incoming radiation. 

Following this decomposition, assimiliation is computed for each LAI layer and is adjusted according to the light environment. It means that each LAI layer is considered as a single leaf (big leaf model). Normally, other parameters will also depend on the canopy position, such as $V_{cmax}$ and $J_{max}$ (see upcoming Lecture 6). However, this is not taken into account in our model representation.

Once assimilation is computed for each LAI layer, we integrate the flux at the canopy scale by weighting the fluxes according to their position into the canopy. Finally, canopy fluxes are integrated at the site level using the available information on the total LAI of the site. 

## Site

We will apply this model to Hyytiala, Finland which is an Evergreen Needleleaf Forests. In other words, it is a land dominated by woody vegetation with a percent cover >60% and height exceeding 2 meters. Almost all trees remain green all year (see site image). Canopy is never without green foliage. The site is characterized by a Mean Annual Temperature of 3.8°C, a Mean Annual Precipitation of 709 mm and a total LAI of 6.7. This site is part of the FLUXNET network, which means that an eddy-covariance tower is installed and measures both meteorological variables and ecosystem CO<sub>2</sub> fluxes every 30 minutes. These are the meteorological data that we will use to force and validate our model. 
...
If you choose to work with the package (not mandatory, only if you feel confortable with it), you can build it by pressing the "Install and Restart" button in the "Build" tab. 
...
Otherwise, you will need to `source` the functions that you need for the practical.
...
* Load the libray needed for the analysis or build the package as described above
* Load the parameters needed to run the model
* Load the meteorological data (file FLUXNET_FI-HYY_2006-2009.csv) provided with the Photosynthesis package
...
# Example to check the parameters for the functions canopy_fluxes
...
siteLAI <- XXX # adapt here (see site description)
...
We ask you to draw (manually) the workflow of the model on a paper and the different calls to the functions. This should contain the different functions and how the functions are called (use arrows). The workflow should also contain the names of main input variables and output variables, and how variables are passed from one module to another. An example of such workflow is the *Algorithm to solve the set of equations* presented above. 
...
Do the same with the site GPP and compare the different years. What can you conclude from the site properties and the quality of data? 

3. Run the photosynthesis/stomatal conductance model for one single year that you will select based on the analysis of the meteorological variable and GPP. 
...
Rerun the model for the same year and compare the gpp simulated for sunlit and shaded leaves. To do so, you need to rebuild the package or resource the `canopy_fluxes` function after having modified it.
